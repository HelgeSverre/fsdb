        ...
        [ testList
          ...
          testList
              ...
              [ testCase "WKT constructors preserve type, coordinates, and SRID through WKB"
                ...
                testList
                    "buffer strategies"
                    [ testCase "use MySQL's binary representation"
                      <| fun _ ->
                      ...
                      <| fun _ ->
                          ...
                          Expect.equal
                              (call
                                  "ST_Equals"
                                  [ mitered
                                    geometry "POLYGON((1 1,2 1,2 3,0 3,-1 3,-1 2,-1 0,1 0,1 1))" ])
                              (VInt 1L)
                              "mitered join"

                      testCase "validate arguments in MySQL's order"
                      <| fun _ ->
                          let geometry text = call "ST_GeomFromText" [ VString text ]
                          let strategy name points = call "ST_Buffer_Strategy" (VString name :: points)
              ...
              [ testCase "NULL sorts before every other value"
                ...
                testTheory
                    ...
                    [ "cp1250", "Zażółć", "5A61BFF3B3E6"
                      ...
                      "armscii8", "Հայ", "D0B3DB"
                      "dec8", "Œÿ", "D7FD"
                      "geostd8", "ქართული", "D8C0D2C8D6CBC9"
                      "gb2312", "中文", "D6D0CEC4"
                      "hp8", "Àÿ", "A1EF"
                      "keybcs2", "Příliš žluťoučký", "50A9A16C69A820916C759F6F75876B98"
                      "sjis", "日本", "93FA967B"
                      "swe7", "ÄÖÅäöå", "5B5C5D7B7C7D"
                      "tis620", "ไทย", "E4B7C2"
                      "ujis", "日本", "C6FCCBDC"
                      "utf16", "😀", "D83DDE00"
                      "utf16le", "😀", "3DD800DE"
                ...
                <| fun _ ->
                    ...
                    Expect.equal (Fsdb.Charset.decodeLoadData "armscii8" [| 0xD0uy; 0xB3uy; 0xDBuy |]) (Ok "Հայ") "ARMSCII-8 file bytes decode strictly"
                    Expect.equal (Fsdb.Charset.decodeLoadData "dec8" [| 0xD7uy; 0xFDuy |]) (Ok "Œÿ") "DEC Multinational bytes decode strictly"
                    Expect.equal (Fsdb.Charset.decodeLoadData "geostd8" [| 0xD8uy; 0xC0uy; 0xD2uy; 0xC8uy; 0xD6uy; 0xCBuy; 0xC9uy |]) (Ok "ქართული") "GEOSTD8 file bytes decode strictly"
                    Expect.equal (Fsdb.Charset.decodeLoadData "gb2312" [| 0xD6uy; 0xD0uy; 0xCEuy; 0xC4uy |]) (Ok "中文") "GB2312 file bytes decode strictly"
                    Expect.equal (Fsdb.Charset.decodeLoadData "hp8" [| 0xA1uy; 0xEFuy |]) (Ok "Àÿ") "HP Roman-8 file bytes decode strictly"
                    Expect.equal (Fsdb.Charset.decodeLoadData "keybcs2" [| 0x50uy; 0xA9uy; 0xA1uy |]) (Ok "Pří") "Kamenický file bytes decode strictly"
                    Expect.equal (Fsdb.Charset.decodeLoadData "sjis" [| 0x93uy; 0xFAuy; 0x96uy; 0x7Buy |]) (Ok "日本") "Shift-JIS file bytes decode strictly"
                    Expect.equal (Fsdb.Charset.decodeLoadData "swe7" [| 0x5Buy; 0x5Cuy; 0x5Duy |]) (Ok "ÄÖÅ") "Swedish IA5 bytes retain their national replacements"
                    Expect.equal (Fsdb.Charset.decodeLoadData "tis620" [| 0xE4uy; 0xB7uy; 0xC2uy |]) (Ok "ไทย") "TIS-620 file bytes decode strictly"
                    Expect.isError (Fsdb.Charset.decodeLoadData "gb2312" [| 0x81uy; 0x40uy |]) "GBK-only bytes are not GB2312"
                    Expect.isError (Fsdb.Charset.decodeLoadData "dec8" [| 0xA4uy |]) "undefined DEC bytes are rejected"
                    Expect.isError (Fsdb.Charset.decodeLoadData "geostd8" [| 0x81uy |]) "undefined GEOSTD8 bytes are rejected"
                    Expect.isError (Fsdb.Charset.decodeLoadData "hp8" [| 0xFFuy |]) "undefined HP Roman-8 bytes are rejected"
                    Expect.isError (Fsdb.Charset.decodeLoadData "sjis" [| 0x87uy; 0x40uy |]) "CP932-only bytes are not Shift-JIS"
                    Expect.isError (Fsdb.Charset.decodeLoadData "sjis" [| 0xFBuy; 0xFCuy |]) "IBM extension bytes are not Shift-JIS"
                    Expect.isError (Fsdb.Charset.decodeLoadData "tis620" [| 0xA0uy |]) "undefined TIS-620 bytes are rejected"
