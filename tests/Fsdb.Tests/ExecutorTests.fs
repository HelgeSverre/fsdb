        ...
        [ testList
          ...
          testList
              ...
              [ testCase "scan-shaped reads preserve direct comparison semantics"
                ...
                testCase "an indexed INNER JOIN probes a composite key beneath an extra equality"
                <| fun _ ->
                    let store = newStore ()
                    runDefault store "CREATE TABLE parent (tenant_id INT, code INT, active INT, PRIMARY KEY (tenant_id, code))" |> ignore
                    ...
                    runDefault store "INSERT INTO parent VALUES (1, 10, 1), (1, 20, 1), (2, 10, 1)" |> ignore
                    runDefault store "INSERT INTO child VALUES (1, 1, 10, 1), (2, 1, 10, 0), (3, 1, 20, 1), (4, 2, 10, 1), (5, 2, 20, 1), (6, NULL, 10, 1)" |> ignore

                    match
                        runDefault
                            store
                            "SELECT p.tenant_id, p.code, c.id FROM parent p INNER JOIN child c ON p.code = c.code AND p.tenant_id = c.tenant_id AND p.active = c.active ORDER BY c.id"
                    with
                    ...
                    match runDefault store "EXPLAIN SELECT * FROM parent p INNER JOIN child c ON p.code = c.code AND p.tenant_id = c.tenant_id AND p.active = c.active" with
