...
let private compareIndexedKeys
    (collations: string option list)
    (directions: Direction list)
    (left: Value list)
    (right: Value list)
    =
    List.zip3 left right (List.zip collations directions)
    |> List.fold (fun comparison (left, right, (collation, direction)) ->
        if comparison <> 0 then
            comparison
        else
            let valueComparison = compareIndexedValues collation left right
            if direction = Desc then -valueComparison else valueComparison) 0

[<CustomEquality; CustomComparison>]
type SecondaryOrderEntry = private { CollationNames: string option list; Directions: Direction list; Values: Value list; RowId: RowId } with
    ...
    override this.Equals other =
        ...
        | :? SecondaryOrderEntry as other ->
            ...
            && compareIndexedKeys this.CollationNames this.Directions this.Values other.Values = 0
            && this.RowId = other.RowId
        | _ -> false

    override this.GetHashCode() = hash (this.CollationNames, this.Directions)
    ...
    interface IComparable with
        member this.CompareTo other =
            ...
            | :? SecondaryOrderEntry as other ->
                ...
                | 0 ->
                    match Operators.compare this.Directions other.Directions with
                    | 0 ->
                        match compareIndexedKeys this.CollationNames this.Directions this.Values other.Values with
...
type private IndexTraversal =
    | Forward
    | Backward

let private oppositeDirection = function
    | Asc -> Desc
    | Desc -> Asc

let private tryIndexTraversal requested stored =
    if requested = stored then
        Some Forward
    elif requested = List.map oppositeDirection stored then
        Some Backward
    else
        None
...
type private SingleColumnKeyGroup =
    { Group: IndexKeyGroup
      ColumnIndex: int
...
let private trySingleColumnKeyGroup group =
    match group.Indices, group.PrefixLengths, group.Transforms, group.Directions with
    | [ columnIndex ], [ prefixLength ], [ transform ], [ direction ] ->
        Some
            { Group = group
        ...
        if columns.IsEmpty then
            ...
            Some
                { Name = index.Name
                  ...
                  PrefixLengths = columns |> List.map (snd >> _.PrefixLength)
                  Transforms = columns |> List.map (snd >> _.Transform)
                  Directions = columns |> List.map (snd >> _.Direction)
...
let private uniqueKeyGroups (table: Table) : IndexKeyGroup list =
    let primary =
        table.Indexes
        |> List.tryFind isPrimaryIndex
        |> Option.bind (tryIndexKeyGroup table)
        |> Option.orElseWith (fun () ->
            ...
            |> Option.map (fun indices ->
                { Name = "PRIMARY"
                  ...
                  Directions = List.replicate indices.Length Asc
                  Visible = true }))
...
let private equalityKeyGroup (index: EqualityIndex) =
    { Name = index.Name
...
let tryUniqueLookup
    ...
    |> Option.bind (fun table ->
        ...
        |> Option.bind (fun index ->
            equalityLookupRows store table index StoredValues [ literal ]
        ...
        | Some lower, Some upper when not requireBound || lower.IsSome || upper.IsSome ->
            ...
            |> Option.map (fun entries ->
                    ...
                    { IndexName = group.Group.Name
                      ColumnIndices = [ index ]
                      PrefixLengths = [ group.PrefixLength ]
                      Directions = [ group.Direction ]
                      Entries = entries
                      First = max 0 first
    ...
    | Backward ->
        seq {
            ...
            while after > slice.First do
                ...
                while
                    first > slice.First
                    && compareIndexedKeys value.CollationNames value.Directions slice.Entries.[first - 1].Values value.Values = 0 do
                    first <- first - 1
...
let private tryOrderedLookup
    ...
    |> Option.bind (fun table ->
        ...
        |> Option.bind (fun slice ->
            ...
            |> List.tryExactlyOne
            |> Option.bind (fun columnIndex ->
                tryIndexTraversal [ direction ] slice.Directions
                |> Option.map (fun traversal -> columnIndex, traversal))
            |> Option.map (fun (columnIndex, traversal) ->
                let rows =
...
let tryEqualityKeyProbe
    ...
    (literal: Value)
    : (Table * EqualityIndex) option =
    tryEqualityKeyProbeForTransform store dbName tableName columnName None literal
...
let private applyAlterAction (mode: TemporalCoercionMode) (table: Table) (action: AlterAction) : Result<Table * string option, StorageError> =
    ...
    let fspCheck =
        ...
        | ChangeColumn(_, col, _) ->
            ...
            validateColumnType col
            |> Result.bind (fun () -> resolveColumn table.Columns oldName)
            |> Result.bind (fun oldIndex ->
                let oldColumn = table.Columns.[oldIndex]
                let effectiveColumn =
                    { col with
                        Name = oldName
                        PrimaryKey = oldColumn.PrimaryKey || col.PrimaryKey
                        Unique = oldColumn.Unique || col.Unique }

                checkVectorKeyColumns [ effectiveColumn ] table.Indexes
                |> Result.bind (fun () -> checkGeometryKeyColumns [ effectiveColumn ] table.Indexes))
        ...
        | AddPrimaryKey cols ->
            let names = cols |> List.map _.Name |> Set.ofList
            let columns = table.Columns |> List.map (fun column -> if Set.contains column.Name names then { column with PrimaryKey = true } else column)

            checkVectorKeyColumns columns []
            |> Result.bind (fun () -> checkGeometryKeyColumns columns [])
        | DropPrimaryKey -> Ok()
        | _ -> Ok()
        ...
        |> Result.bind (fun idxs ->
            let group =
                { Name = ix.Name
                  ...
                  Directions = ix.KeyColumns |> List.map _.Direction
                  Visible = ix.Visible }
    ...
    | SetIndexVisibility(name, visible) ->
        ...
        match table.Indexes |> List.tryFind equal with
        | None -> Error(ExpressionError(1176, sprintf "Key '%s' doesn't exist in table '%s'" name table.OriginalName))
        | Some index when isPrimaryIndex index && not visible ->
            Error(ExpressionError(3522, "A primary key index cannot be invisible"))
        | Some _ ->
            Ok(
                { table with
                    Indexes = table.Indexes |> List.map (fun index -> if equal index then { index with Visible = visible } else index) },
                None
            )
    ...
    | AddPrimaryKey cols ->
        if table.Columns |> List.exists _.PrimaryKey then
            ...
            cols
            |> traverse (_.Name >> resolveColumn table.Columns)
...
let private finalizePreparedAutoValue
    ...
    =
    match table.Columns |> List.tryFindIndex _.AutoIncrement with
    | None -> nextAutoId, assigned, false
    | Some index when not (Set.contains index deferred) && before.[index] = candidate.[index] -> nextAutoId, assigned, false
    | Some index ->
        let column = table.Columns.[index]

        if generatesAutoValue mode generateAutoOnZero column candidate.[index] then
            candidate.[index] <- VInt nextAutoId
            (if nextAutoId = Int64.MaxValue then Int64.MaxValue else nextAutoId + 1L), Some(true, nextAutoId), true
        else
            let explicitValue =
            ...
            if values.Length <> indices.Length then
                ...
                |> Result.bind (fun (processed, nextAutoId, assignedAutoId, omitted) ->
                    ...
                    |> Result.bind (fun candidate ->
                        let nextAutoId, assignedAutoId, candidateChanged =
                        ...
                        (if candidateChanged then finish candidate else Ok candidate)
...
let internal prepareInsertCandidateWithDeferred store dbName tableName columns values deferred prepare finish =
    prepareInsertCandidateCore store dbName tableName columns values deferred prepare finish
            ...
            if List.length rowValues <> List.length idxs then
                ...
                let rowResult =
                    ...
                    |> Result.bind (fun (finalValues, nextAutoId', assigned, omitted) ->
                        ...
                        |> Result.bind (fun candidate ->
                            let nextAutoId', assigned, candidateChanged =
                                finalizePreparedAutoValue
                                    mode
                                    generateAutoOnZero
                                    table
...
let insertRows
    ...
    (rowsIn: Value list list)
    : Result<InsertOutcome, StorageError> =
    insertRowsPreparedCore false Set.empty (fun _ _ row -> Ok row) (fun _ row -> Ok row) store dbName tableName columns rowsIn
...
let insertRowsPrepared
    ...
    (finish: Value[] -> Result<Value[], StorageError>)
    : Result<InsertOutcome, StorageError> =
    insertRowsPreparedCore false Set.empty (fun _ omitted row -> prepare omitted row) (fun _ row -> finish row) store dbName tableName columns rowsIn
...
let internal insertRowsPreparedWithOrdinal
    ...
    (finish: int -> Value[] -> Result<Value[], StorageError>)
    : Result<InsertOutcome, StorageError> =
    insertRowsPreparedCore false deferred prepare finish store dbName tableName columns rowsIn
...
let insertRowsIgnore
    ...
    (rowsIn: Value list list)
    : Result<InsertOutcome, StorageError> =
    insertRowsPreparedCore true Set.empty (fun _ _ row -> Ok row) (fun _ row -> Ok row) store dbName tableName columns rowsIn
...
let insertRowsIgnorePrepared
    ...
    (finish: Value[] -> Result<Value[], StorageError>)
    : Result<InsertOutcome, StorageError> =
    insertRowsPreparedCore true Set.empty (fun _ omitted row -> prepare omitted row) (fun _ row -> finish row) store dbName tableName columns rowsIn
...
let internal insertRowsIgnorePreparedWithOrdinal
    ...
    (finish: int -> Value[] -> Result<Value[], StorageError>)
    : Result<InsertOutcome, StorageError> =
    insertRowsPreparedCore true deferred prepare finish store dbName tableName columns rowsIn
...
let replaceRows
    ...
    (prepare: Set<int> -> Value[] -> Result<Value[], StorageError>)
    : Result<InsertOutcome, StorageError> =
    replaceRowsCore store dbName tableName columns rowsIn Set.empty (fun _ omitted row -> prepare omitted row) (fun _ row -> Ok row)
