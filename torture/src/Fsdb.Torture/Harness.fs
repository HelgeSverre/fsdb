...
module AstKind =
    let rec ofStatement =
        ...
        | CreateServer _ -> "create_server"
        | AlterServer _ -> "alter_server"
        | DropServer _ -> "drop_server"
        ...
        | SetRole _ -> "set_role"
        | SetDefaultRole _ -> "set_default_role"
...
module Database =
    ...
    let private addParameters (command: MySqlCommand) (parameters: obj array) =
        parameters
    ...
    let private executeCommandCore prepare (target: string) timeoutSeconds (command: MySqlCommand) =
        task {
            ...
            try
                if prepare then
                    do! command.PrepareAsync(timeout.Token)
    ...
    let all =
        ...
        | Scalar ->
            [| "scalar_bounds",
               ...
               "spatial_independent_round_strategies",
               "SELECT ST_IsValid(ST_Buffer(ST_GeomFromText('LINESTRING(0 0,0 2,2 2)'), 1, ST_Buffer_Strategy('end_round',5), ST_Buffer_Strategy('join_round',7))) AS valid_mixed_buffer, ST_Contains(ST_Buffer(ST_GeomFromText('POLYGON((0 0,4 0,4 4,0 4,0 0))'), 1, ST_Buffer_Strategy('join_round',5.5)), ST_GeomFromText('POINT(-0.5 -0.5)')) AS contains_rounded_corner"
               ...
               "SELECT /*+ SET_VAR(max_points_in_geometry=32) */ @@SESSION.max_points_in_geometry AS persistent_limit, HEX(ST_Buffer_Strategy('point_circle', 32)) AS strategy_bytes"
        ...
        | Relational ->
            [| "project_tenant_orphans",
               ...
               "SELECT COUNT(*) AS limited_pairs FROM (SELECT a.id FROM tasks AS a JOIN tasks AS b ON b.status = a.status WHERE a.id <= 50 LIMIT 25) AS limited"
