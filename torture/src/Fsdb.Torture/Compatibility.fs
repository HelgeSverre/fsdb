...
module ContractCatalog =
    ...
    let private columnTypes =
        ...
        let insert =
            "INSERT INTO contract_types VALUES (1, -8, TRUE, -32000, 8000000, -2000000000, 9000000000, "
        ...
        let select columns parameter =
            sprintf "SELECT %s FROM contract_types WHERE id = %s" columns parameter

        let allColumns =
            "SELECT c_tiny, c_bool, c_small, c_medium, c_int, c_big, c_bit, c_char, c_varchar, c_tinytext, c_text, c_mediumtext, c_longtext, "
            + "c_binary, c_varbinary, c_tinyblob, c_blob, c_mediumblob, c_longblob, c_enum, c_set, c_decimal, c_double, c_float, "
            + "c_date, c_datetime, c_timestamp, c_time, c_year, c_json, c_geometry FROM contract_types WHERE id = 1"

        { Name = "column-type-roundtrip"
          Setup = [| "DROP TABLE IF EXISTS contract_types"; createTable; insert |]
          Steps =
            [| Contract.query "text-type-row" allColumns
               Contract.preparedQuery
                   "prepared-numeric-row"
                   (select "c_tiny, c_bool, c_small, c_medium, c_int, c_big, c_bit, c_decimal, c_double, c_float, c_year" "?")
                   [| box 1 |]
               Contract.preparedQuery
                   "prepared-text-binary-row"
                   (select "c_char, c_varchar, c_tinytext, c_text, c_mediumtext, c_longtext, c_binary, c_varbinary, c_tinyblob, c_blob, c_mediumblob, c_longblob, c_enum, c_set, c_json" "?")
                   [| box 1 |]
               Contract.preparedQuery
                   "prepared-temporal-row"
                   (select "c_date, c_datetime, c_timestamp, c_time" "?")
                   [| box 1 |]
               Contract.preparedQuery "prepared-geometry-row" (select "c_geometry" "?") [| box 1 |] |]
