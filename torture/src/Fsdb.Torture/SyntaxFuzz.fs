...
module SyntaxFuzz =
    ...
    let private seeds suffix =
        [| "row_constructor", "SELECT ROW(1, 2) = ROW(1, 2)"
           ...
           "fulltext_ordered_unique_join",
           "SELECT f.id, t.label FROM syntax_fulltext AS f JOIN syntax_target AS t ON t.id = f.id WHERE MATCH(f.title, f.body) AGAINST ('database') LIMIT 1"
           ...
           "role_grant", "GRANT 'syntax_parent'@'%' TO 'syntax_role_delegate'@'%' WITH ADMIN OPTION"
           "role_default", "SET DEFAULT ROLE 'syntax_parent'@'%' TO 'syntax_role_user'@'%'"
           ...
           "spatial_line_buffer",
           "SELECT ST_Contains(ST_Buffer(ST_GeomFromText('LINESTRING(0 0,2 0)'), 1), ST_GeomFromText('POINT(1 0)'))"
           "spatial_overlay",
    ...
    let private fixtures =
        [| "CREATE TABLE syntax_target (id INT PRIMARY KEY, n INT, label VARCHAR(40), INDEX ix_n (n), INDEX ix_n_label (n, label))"
           ...
           "CREATE ROLE 'syntax_reader'@'%', 'syntax_parent'@'%'"
           "CREATE USER 'syntax_role_user'@'%', 'syntax_role_delegate'@'%'"
           "GRANT SELECT ON syntax_target TO 'syntax_reader'@'%'"
           "GRANT 'syntax_reader'@'%' TO 'syntax_parent'@'%'"
           "GRANT 'syntax_parent'@'%' TO 'syntax_role_user'@'%' WITH ADMIN OPTION"
