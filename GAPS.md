...
| Depth-three syntax stress | Three earlier 10,000-mutation seeds produced no crash, timeout, protocol fault, or invariant failure. The 2026-09-01 maximum run matched 8,340 errors and accepted 1,312 mutations, while exposing four contracts: three malformed `CALL` forms return fsdb 1305 instead of MySQL 1064, and one valid multiline `mandatory_roles` assignment is rejected. The harness unit suite also reports that `procedure_recursion` no longer receives a successful dense-comment mutation | four parser contracts open; harness coverage assertion open |
...
| Crash/restart durability | Concurrent two-table transactions were interrupted by 80 forced process crashes across four 16-worker campaigns. Recovery retained every acknowledged commit, exposed no partial transaction, invented no row, and preserved identical state through graceful snapshot restarts; the latest 20-restart campaign retained all 672 acknowledged operations and reconciled 320 ambiguous operations into 675 recovered commits without a partial transaction | resolved 2026-09-01; broader snapshot-rotation volume remains useful stress coverage |
