...
| Column definition fidelity | schema/table/org_table names, requested charsetnr | direct physical COM_QUERY/COM_STMT_PREPARE columns and COM_FIELD_LIST report source names, while declared columns and explicit COLLATE expressions report their MySQL collation ids; view, derived, UNION, and untyped text metadata retains empty source names or protocol fallback 45 | low | partial |
