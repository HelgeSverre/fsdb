...
| Multi-table `JSON_TABLE` writes | read-only emitted rows can filter physical UPDATE/DELETE targets | `JSON_TABLE` participates in reads but is rejected as an UPDATE or DELETE join source | low | refusal |
