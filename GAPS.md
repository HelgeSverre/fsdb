...
| Planner slope rerun | The 2026-09-01 10k/50k follow-up retains sub-millisecond indexed equality/range probes. Reverse PK probing reduced `RIGHT JOIN ... LIMIT 50` from 249 ms to 364 µs versus MySQL's 220 µs and remains flat at 389 µs versus 308 µs on 100k/500k. Reusing compatible window order, memoizing snapshot rows, bounded partition selection, and direct peer counts reduced `WindowTopOrders` from 325 ms to 83.5 ms versus 50.4 ms and `CUME_DIST` from 84.5 ms to 11.0 ms versus 18.7 ms. Grouping is 24.1 ms versus 19.8 ms. Deferring safe direct projections until bounded top-N selects its survivors reduced the non-indexed filter/sort/limit scan from 8.93 ms to 5.31 ms versus 1.93 ms. Fusing untouched built-in `JSON_UNQUOTE(JSON_EXTRACT(...))` calls reduced JSON extraction from 281 µs to 180 µs versus 62.2 µs while preserving registry overrides. Stable row identities reduced the non-indexed single-row UPDATE from 11.35 ms to 6.53 ms versus 3.58 ms. Direct indexed correlated counts reduced the 100-row query from 1.83 ms to 1.03 ms versus 208 µs. Exact `information_schema.COLUMNS` construction reduced the structure-pane query from 1.63 ms to 191 µs versus 249 µs. Candidate reuse, retained row ids, and direct single-term score dictionaries reduced the full-text join from 10.9 ms to 1.78 ms versus 430 µs and natural search from 2.33 ms to 874 µs versus 412 µs | measured slope cliffs resolved; constant factors remain |
