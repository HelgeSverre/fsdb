...
| Planner slope rerun | a focused 10k/50k ShortRun measured the indexed join at 371 µs versus MySQL's 178 µs and uncorrelated `IN` at 534 µs versus 155 µs, replacing the historical 4.49 ms and 103 ms fsdb results. `GROUP BY` remained 98.2 ms versus 19.9 ms and the 50k-row window remained 354 ms versus 49.3 ms | join/subquery cliffs resolved; aggregate/window constants open |
