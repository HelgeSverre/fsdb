...
| Planner slope rerun | the 2026-08-31 10k/50k quick matrix kept indexed equality/range probes in the sub-millisecond range and exposed one asymmetric outer-join cliff: `RIGHT JOIN ... LIMIT 50` took 249 ms versus MySQL's 245 µs while the equivalent indexed LEFT path took 746 µs. Reverse probing the left PK now streams that preserved right side at 364 µs versus MySQL's 220 µs; at 100k/500k it stays flat at 389 µs versus 308 µs. Window execution remains 325 ms versus 66.0 ms for `ROW_NUMBER` and 145 ms versus 22.4 ms for `CUME_DIST`; full-text joins remain 10.9 ms versus 422 µs | outer-join cliff resolved; window and full-text constants open |
