...
| Gap | MySQL 8.4 | fsdb | Impact | Class |
|---|---|---|---|---|
| Non-unique secondary indexes | physical structures serving lookups/ordering | separate immutable equality buckets and ordered entries serve fully-bound composite equality, scalar and composite-row literal `IN` lists, prefix-key candidates with full residual checks, matching physical inner/left/right-join keys, direct literal SELECT/UPDATE/DELETE ranges, compatible grouping, composite stored-column index ordering after an optional literal-equality prefix, and matching single-part `LOWER(column)`/`UPPER(column)` ordering; duplicate structures deliberately trade memory and write work for point probes plus bounded seeks; other expression ordering still sorts | high (scale) | divergence |
...
| Expression indexes | functional key parts participate in physical access and uniqueness | `LOWER(column)` and `UPPER(column)` indexes maintain physical equality buckets for matching equality and literal-`IN` SELECT/DML predicates, maintain ordered entries for a matching single-expression `ORDER BY`, and enforce uniqueness; other non-unique expressions retain DDL, persistence, and metadata but use scan fallback, while other unique expressions are refused | low | divergence/refusal |
