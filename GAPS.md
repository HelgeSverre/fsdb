...
| Gap | MySQL 8.4 | fsdb | Impact | Class |
|---|---|---|---|---|
| Secondary-index access paths | ref/eq_ref/range scans feed joins, DML, ORDER BY, GROUP BY | fully-bound composite equality probes and matching physical inner/left joins use B-tree buckets; one-column literal `IN` lists and direct literal ranges feed single-table SELECT/UPDATE/DELETE and locking reads through primary, unique, and secondary indexes; bounded `ORDER BY`/`GROUP BY` can stream a matching composite index when preceding keys are fixed; right joins, unconstrained multi-key ordering, and broader grouping still scan/sort | high (scale) | divergence |
...
| Gap | MySQL 8.4 | fsdb | Impact | Class |
|---|---|---|---|---|
| Non-unique secondary indexes | physical structures serving lookups/ordering | separate immutable equality buckets and ordered entries serve fully-bound composite equality, one-column literal `IN` lists, prefix-key candidates with full residual checks, matching physical inner/left-join keys, direct literal SELECT/UPDATE/DELETE ranges, compatible grouping, and bounded composite index ordering; duplicate structures deliberately trade memory and write work for point probes plus bounded seeks; right joins and unconstrained ordering remain scans | high (scale) | divergence |
