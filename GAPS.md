...
| Gap | MySQL 8.4 | fsdb | Impact | Class |
|---|---|---|---|---|
| Secondary-index access paths | ref/eq_ref/range/index-merge scans feed joins, DML, ORDER BY, GROUP BY | common complete-key and safe left-prefix equality, literal membership, range, join, ordering, grouping, supported unary functional compositions, correlated functional equality, numeric-result functional ranges, fully covered OR unions, and cost-effective AND intersections use maintained indexes; text-result expression ranges, arbitrary expression ordering, and broader grouping still scan or sort | high (scale) | divergence |
...
| Subquery strategies | semi-join/materialization/early-exit transformations | stable subqueries materialize once; common correlated equality/range shapes, supported functional equalities, and numeric-result functional ranges over physical or pass-through projected sources probe maintained indexes; variable-bearing, nondeterministic, lateral, JSON_TABLE, and more complex correlated forms re-execute | medium (scale) | divergence |
