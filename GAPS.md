...
| Planner slope rerun | the 2026-08-31 10k/50k quick matrix kept indexed equality/range probes in the sub-millisecond range and exposed one asymmetric outer-join cliff: `RIGHT JOIN ... LIMIT 50` took 249 ms versus MySQL's 245 µs while the equivalent indexed LEFT path took 746 µs. Reverse probing the left PK now streams that preserved right side at 364 µs versus MySQL's 220 µs; at 100k/500k it stays flat at 389 µs versus 308 µs. Reusing compatible `ROW_NUMBER` output order and removing duplicate `WHERE` evaluation reduced that window case from 325 ms to 213 ms versus MySQL's 60.0 ms. Dense peer counting and prepared direct-column order terms reduced low-cardinality `CUME_DIST` from 145 ms to 102 ms versus MySQL's 19.4 ms. Candidate reuse and skipping unused DISTINCT keys reduced the full-text join from 10.9 ms to 2.91 ms versus MySQL's 422 µs; natural search is 1.35 ms versus 396 µs | outer-join cliff resolved; window and full-text constants open |
