...
| Routines & events | Typed IN/OUT/INOUT procedures with compound variables, condition handlers, loops, multi-result CALL, and one-time event declarations | Stored functions, cursors, diagnostics statements, and event scheduling |
...
|---|---|---|
| Procedures support typed `IN`/`OUT`/`INOUT` parameters, scoped variables, compound control flow, condition handlers, `SIGNAL`/`RESIGNAL`, and multi-result CALL; stored functions, cursors, dynamic SQL, and `GET DIAGNOSTICS` remain absent | medium | divergence/refusal |
...
| Gap | MySQL 8.4 | fsdb | Impact | Class |
|---|---|---|---|---|
| Routine language | procedures/functions, compound bodies, handlers, cursors, loops, CASE, SIGNAL, diagnostics, and dynamic SQL | typed procedure parameters, scoped local variables and conditions, CONTINUE/EXIT handlers, SIGNAL/RESIGNAL, branches, labeled loops, sequential statements, and multi-result CALL are covered; stored functions, cursors, GET DIAGNOSTICS, and dynamic SQL remain absent | medium | refusal |
...
3. Complex join-derived updatable views and the remaining stored-program
   ...
   functions, cursors, GET DIAGNOSTICS, and dynamic SQL remain absent.
