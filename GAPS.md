...
| Expression indexes | functional key parts participate in physical access and uniqueness | direct-column `LOWER`/`LCASE`, `UPPER`/`UCASE`, `TRIM`, `REVERSE`, `CHAR_LENGTH`/`CHARACTER_LENGTH`, `LENGTH`/`OCTET_LENGTH`, `BIT_LENGTH`, and numeric `ABS` key parts maintain physical equality buckets for matching equality and literal-`IN` SELECT/DML predicates, maintain ordered entries for matching `ORDER BY` and `GROUP BY` prefixes and fixed-prefix suffixes, and enforce uniqueness; byte-length keys honor the source column charset; coercing text `ABS` and other non-unique expressions retain DDL, persistence, and metadata but use scan fallback, while other unique expressions are refused | low | divergence/refusal |
