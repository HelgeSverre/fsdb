...
| Prefix indexes | `INDEX (col(N))` with SUB_PART metadata | DDL, persistence, size validation, SHOW, INFORMATION_SCHEMA, UNIQUE enforcement, and single/composite equality probes for SELECT/DML/inner joins use character- or byte-prefix keys with complete residual checks; prefix ranges and ordering still scan/sort | low | divergence |
