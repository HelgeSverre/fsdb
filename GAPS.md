...
family with match_type), math (ROUND with exact/approximate split, CONV,
...
| Time arithmetic | `ADDTIME SUBTIME TIMEDIFF SEC_TO_TIME MAKETIME TIME_FORMAT PERIOD_ADD PERIOD_DIFF FROM_DAYS TO_DAYS UTC_DATE UTC_TIME UTC_TIMESTAMP GET_FORMAT SYSDATE`; aliases `LOCALTIME[STAMP]`; seeded `RAND(N)` | medium |
...
| Misc | `BENCHMARK SLEEP UUID_SHORT ANY_VALUE COERCIBILITY NAME_CONST DEFAULT()` outside REPLACE-SET, `SOUNDEX MAKE_SET MID ORD TO_BASE64 FROM_BASE64 BIT_LENGTH SHA` alias | low |
