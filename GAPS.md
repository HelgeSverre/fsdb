...
| XA recovery details | recovered branches retain InnoDB locks and `XA RECOVER` requires `XA_RECOVER_ADMIN` | live prepared branches retain row/key ownership; after restart, overlapping completion returns 1205 through optimistic validation instead of waiting on reconstructed locks; `SUPER` is the recovery gate until dynamic privilege grants exist; use `CONVERT XID` for byte-exact non-ASCII identifiers because the unconverted result still crosses the string result carrier | low | divergence |
