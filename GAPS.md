...
| Constraints & indexes | PK/UNIQUE/FK/CHECK plus composite equality, inner/left/right joins, PK/unique/secondary range, grouping, and index-order probes | Expression ordering and broader grouping paths still scan |
...
`IN`/`= ANY` and row-value `IN` materializations with reusable membership sets and direct indexed
...
| Gap | MySQL 8.4 | fsdb | Impact | Class |
|---|---|---|---|---|
| Secondary-index access paths | ref/eq_ref/range scans feed joins, DML, ORDER BY, GROUP BY | fully-bound composite equality probes, scalar/composite literal `IN` lists, and matching physical inner/left/right joins use B-tree buckets; direct literal ranges feed single-table SELECT/UPDATE/DELETE and locking reads through primary, unique, and secondary indexes; `ORDER BY` can stream any whole-column left prefix of a compatible composite index, and `GROUP BY` can stream a matching index when preceding keys are fixed; expression ordering and broader grouping still scan/sort | high (scale) | divergence |
...
| Gap | MySQL 8.4 | fsdb | Impact | Class |
|---|---|---|---|---|
| Non-unique secondary indexes | physical structures serving lookups/ordering | separate immutable equality buckets and ordered entries serve fully-bound composite equality, scalar and composite-row literal `IN` lists, prefix-key candidates with full residual checks, matching physical inner/left/right-join keys, direct literal SELECT/UPDATE/DELETE ranges, compatible grouping, and left-prefix composite index ordering; duplicate structures deliberately trade memory and write work for point probes plus bounded seeks; expression ordering still sorts | high (scale) | divergence |
| Prefix indexes | `INDEX (col(N))` with SUB_PART metadata | DDL, persistence, size validation, SHOW, INFORMATION_SCHEMA, UNIQUE enforcement, and equality/range probes for SELECT/DML/inner/left/right joins use character- or byte-prefix keys with complete residual checks; full-value ordering still sorts | low | divergence |
