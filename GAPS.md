...
materialized once per statement, compatible integer/string/decimal `IN`
materializations with a reusable membership set and direct indexed outer-table narrowing, correlated
...
| Gap | MySQL 8.4 | fsdb | Impact | Class |
|---|---|---|---|---|
| Secondary-index access paths | ref/eq_ref/range scans feed joins, DML, ORDER BY, GROUP BY | fully-bound composite equality probes, scalar/composite literal `IN` lists, and matching physical inner/left joins use B-tree buckets; direct literal ranges feed single-table SELECT/UPDATE/DELETE and locking reads through primary, unique, and secondary indexes; bounded `ORDER BY`/`GROUP BY` can stream a matching composite index when preceding keys are fixed; right joins, unconstrained multi-key ordering, and broader grouping still scan/sort | high (scale) | divergence |
...
| Subquery strategies | semi-join/materialization/early-exit transformations | statement-stable scalar/IN/ANY/SOME/ALL/EXISTS subqueries materialize once; compatible integer/string/decimal `IN` reuses a typed equality set and narrows a direct indexed physical outer table, retaining residual predicates and prefix-key checks; simple EXISTS stops at one row; direct correlated equalities use persistent indexes or a statement-local canonical-key lookup over a physical inner table; compound semi-joins remain scans, while other correlated, variable-bearing, nondeterministic, CTE, derived, lateral, and JSON_TABLE forms re-execute | medium (scale) | divergence |
...
| Gap | MySQL 8.4 | fsdb | Impact | Class |
|---|---|---|---|---|
| Non-unique secondary indexes | physical structures serving lookups/ordering | separate immutable equality buckets and ordered entries serve fully-bound composite equality, scalar and composite-row literal `IN` lists, prefix-key candidates with full residual checks, matching physical inner/left-join keys, direct literal SELECT/UPDATE/DELETE ranges, compatible grouping, and bounded composite index ordering; duplicate structures deliberately trade memory and write work for point probes plus bounded seeks; right joins and unconstrained ordering remain scans | high (scale) | divergence |
| Prefix indexes | `INDEX (col(N))` with SUB_PART metadata | DDL, persistence, size validation, SHOW, INFORMATION_SCHEMA, UNIQUE enforcement, and equality/range probes for SELECT/DML/inner and left joins use character- or byte-prefix keys with complete residual checks; full-value ordering still sorts | low | divergence |
| Expression indexes | functional key parts participate in physical access and uniqueness | `LOWER(column)` indexes maintain physical equality buckets for matching equality and literal-`IN` SELECT/DML predicates and enforce uniqueness; other non-unique expressions retain DDL, persistence, and metadata but use scan fallback, while other unique expressions are refused | low | divergence/refusal |
...
| LOAD DATA LOCAL INFILE | client-streamed file loading | opt-in `local_infile`; UTF-8/utf8mb4, one-character field/line separators, `REPLACE`/`IGNORE`, header skipping, column/user-variable fields, and ordered `SET` transformations; no server-file loading or multibyte separators | low | subset |
