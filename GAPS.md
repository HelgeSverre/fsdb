...
| Multi-table `JSON_TABLE` writes | emitted rows retain the identity of an underlying writable target | `JSON_TABLE` participates in reads but is rejected as an UPDATE or DELETE join source | low | refusal |
...
| Scalar substring search | `LOCATE`, `INSTR`, and `POSITION` apply operand collation and coercibility | these scalar functions use the server-default collation because their value-only extension signature carries no operand metadata | low | divergence |
...
| Arithmetic overflow text | error 1690 names the offending expression | the error code and numeric domain match, but the value-layer exception does not carry an originating expression | low | divergence |
