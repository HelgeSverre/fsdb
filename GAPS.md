...
| Expression indexes | functional key parts participate in physical access and uniqueness | direct-column `LOWER`/`LCASE`, `UPPER`/`UCASE`, `TRIM`, `REVERSE`, and `CHAR_LENGTH`/`CHARACTER_LENGTH` key parts maintain physical equality buckets for matching equality and literal-`IN` SELECT/DML predicates, maintain ordered entries for matching `ORDER BY` and `GROUP BY` prefixes and fixed-prefix suffixes, and enforce uniqueness; other non-unique expressions retain DDL, persistence, and metadata but use scan fallback, while other unique expressions are refused | low | divergence/refusal |
