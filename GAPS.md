...
| Planner slope rerun | the 2026-08-31 10k/50k quick matrix kept indexed equality/range probes in the sub-millisecond range and exposed one asymmetric outer-join cliff: `RIGHT JOIN ... LIMIT 50` took 249 ms versus MySQL's 245 µs while the equivalent indexed LEFT path took 746 µs. Reverse probing the left PK now streams that preserved right side at 364 µs versus MySQL's 220 µs; at 100k/500k it stays flat at 389 µs versus 308 µs. Reusing compatible `ROW_NUMBER` output order, removing duplicate `WHERE` evaluation, and preparing direct order terms reduced `WindowTopOrders` from 325 ms to 136 ms. Restoring memoized snapshot row lists outside actual locking reads removed an accidental full-table list copy and brought it to 123 ms versus MySQL's 48.5 ms; `CUME_DIST` is 84.5 ms versus 18.4 ms, grouping 25.0 ms versus 20.2 ms, and JSON extraction 276 µs versus 62.6 µs. Retaining stable row ids from predicate scan through storage publication reduced the 10k-row non-indexed single-row UPDATE from 11.35 ms to 6.77 ms versus MySQL's 3.61 ms. Directly counting correlated equality-index candidates reduced the 100-row correlated count from 1.83 ms to 1.15 ms versus MySQL's 205 µs. Exact `information_schema.COLUMNS` lookup now constructs only the requested table's rows, reducing the structure-pane query from 1.63 ms to 231 µs versus MySQL's 255 µs. Candidate reuse and skipping unused DISTINCT keys reduced the full-text join from 10.9 ms to 2.43 ms versus MySQL's 417 µs; natural search is 1.32 ms versus 400 µs | outer-join, grouping, and exact metadata cliffs resolved; window and full-text constants open |
