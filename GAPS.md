...
| Gap | MySQL 8.4 | fsdb | Impact | Class |
|---|---|---|---|---|
| Secondary-index access paths | ref/eq_ref/range scans feed joins, DML, ORDER BY, GROUP BY | fully-bound composite equality probes, scalar/composite literal `IN` lists, and matching physical inner/left/right joins use B-tree buckets; direct literal ranges feed single-table SELECT/UPDATE/DELETE and locking reads through primary, unique, and secondary indexes; `ORDER BY` can stream any whole-column left prefix, a suffix after literal-equality keys, or a matching single-part `LOWER(column)`/`UPPER(column)` index, and `GROUP BY` can stream the same stored-column prefixes and single-part case-folding keys; arbitrary expression ordering and broader grouping still scan/sort | high (scale) | divergence |
...
| Gap | MySQL 8.4 | fsdb | Impact | Class |
|---|---|---|---|---|
| Non-unique secondary indexes | physical structures serving lookups/ordering | separate immutable equality buckets and ordered entries serve fully-bound composite equality, scalar and composite-row literal `IN` lists, prefix-key candidates with full residual checks, matching physical inner/left/right-join keys, direct literal SELECT/UPDATE/DELETE ranges, compatible stored-column grouping, composite stored-column index ordering after an optional literal-equality prefix, and matching single-part `LOWER(column)`/`UPPER(column)` ordering and grouping; duplicate structures deliberately trade memory and write work for point probes plus bounded seeks; other expression ordering and grouping still sort | high (scale) | divergence |
...
| Expression indexes | functional key parts participate in physical access and uniqueness | `LOWER(column)` and `UPPER(column)` indexes maintain physical equality buckets for matching equality and literal-`IN` SELECT/DML predicates, maintain ordered entries for matching single-expression `ORDER BY` and `GROUP BY`, and enforce uniqueness; other non-unique expressions retain DDL, persistence, and metadata but use scan fallback, while other unique expressions are refused | low | divergence/refusal |
