...
| HASH and LINEAR HASH partition definitions, `pN` selection, INFORMATION_SCHEMA/SHOW metadata, and `ADD`/`COALESCE PARTITION` are logical catalog features over the shared row store; `DROP PARTITION` returns MySQL's HASH-specific refusal, while physical pruning, `TRUNCATE PARTITION`, partition maintenance clauses, and `REORGANIZE PARTITION` remain absent | low | divergence/refusal |
