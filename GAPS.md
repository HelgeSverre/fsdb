...
| System-table coverage | 38 mysql.* tables with engine-maintained contents | all 38 table names and MySQL 8.4 column order, types, nullability, key membership, defaults, and generated columns exist, plus six fsdb stored-object catalogs; the eight default optimizer-cost rows are present, but native catalog collations and engine-maintained help, log, GTID, InnoDB-statistics, procedure-grant, NDB, and replication-state rows still differ or remain empty unless ordinary fsdb DML populates them | low | divergence |
