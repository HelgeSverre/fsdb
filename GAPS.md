...
| Column definition fidelity | schema/table/org_table names, requested charsetnr | direct physical and view COM_QUERY/COM_STMT_PREPARE columns plus COM_FIELD_LIST report source names; declared expressions and text-probed resultsets report their effective MySQL collation ids; derived-table source names remain empty, while UNION source names are empty in MySQL too | low | partial |
