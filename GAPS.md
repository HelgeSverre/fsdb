...
| HASH and LINEAR HASH partition definitions, `pN` selection, INFORMATION_SCHEMA/SHOW metadata, `ADD`/`COALESCE PARTITION`, and logical `ANALYZE`/`CHECK`/`OPTIMIZE`/`REPAIR PARTITION` are catalog features over the shared row store; `DROP PARTITION` returns MySQL's HASH-specific refusal, while physical pruning, `TRUNCATE PARTITION`, and `REORGANIZE PARTITION` remain absent | low | divergence/refusal |
